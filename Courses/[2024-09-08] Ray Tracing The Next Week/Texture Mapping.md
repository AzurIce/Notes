...
而由于在按照 $-Z \to +X \to +Z \to -X \to -Z$ 的顺序取值时，相当于是按照 第一象限 -> 第二象限 -> 第三象限 -> 第四象限 取值的，因此 $\text{atan2}$ 的输出先是 $0 \to \pi$ 然后是 $-\pi \to 0$，而不是连续的 $-\pi \to \pi$，所以对输入的两个自变量都取个反（相当于把上图左右上下全部对称一下）就可以得到连续地 $-\pi \to \pi$，而最终我们想要的是 $0 \to 2\pi$，那也很简单，再加上 $\pi$ 即可
$$
\begin{cases}
z = \sin\phi \cdot \sin\theta\\
-x = \cos\phi \cdot \sin\theta
